...
`requirements.txt` reuses the sibling LLM gateway through `-e ../llm_gateway`.
Keep `agent_roundtable/` and `llm_gateway/` as sibling directories locally; identity, transport, model discovery, and fallback-chain execution are maintained only in the latter.
...
DeepSeek uses an OpenAI-compatible API with `base_url=https://api.deepseek.com`. Both live discovery and the UI's offline fallback come from `llm_gateway`; this repository no longer maintains provider model or key-name registries.
...
Real keys still come only from `.env`. `config/model_example.json` is the complete snapshot generated by `llm-gateway refresh-example`; the UI reads it only when live model discovery is off.
...
4. If a route omits its model, use the provider default owned by `llm_gateway`.
...
├── llm/                     # Thin roundtable-owned business adapters
│   ├── facade.py            # Adapt gateway chains to LLMClient.generate
│   ├── router.py            # Gateway dispatch plus the MockLLM seam
│   └── providers_api.py     # LLMClient protocol, MockLLM, display metadata
├── config/
...
│   ├── agent_llms.json
│   └── model_example.json   # Complete gateway-generated model snapshot
...
Each index directory contains `chunks.jsonl` plus `index_manifest.json`, which records the corpus scope, source files, content fingerprint, and index version. After renaming or deleting corpus directories, rebuild and prune stale generated indexes with:
...
The same core checks run in GitHub Actions on Python 3.10 and 3.13:
...
Launch the UI separately when changing Streamlit behavior:
